...
export default class App extends Component {
  ...
  cnv = null 
  options = " .`-_':,;^=+/\"|)\\<>)iv%xclrs{*}I?!][1taeo7zjLunT#JCwfy325Fp6mqSghVd4EgXPGZbYkOA&8U$@KHDBWNMR0Q"
  gui = null
  btn = null
  livebtn = null
  live = true
  ...
  kernel = this.masks.sharp
  dest = null;

  preload = (p5) => {
    this.img = p5.loadImage("https://4.bp.blogspot.com/-mLOwpEsNL4Y/UCu0wcVsPBI/AAAAAAAAA6s/7ECKTpxXr3o/s1600/lena.bmp")
  }

  setup = (p5, canvasParentRef) => {
    this.cvn = p5.createCanvas(this.img.width,this.img.height).parent(canvasParentRef)
    p5.pixelDensity(1)
  ...
  apply = p => {
    ...
    for (let x = 0; x < this.dest.width; x++) {
      for (let y = 0; y < this.dest.height; y++) {
        let result = this.convolution(p,this.img, x, y, this.kernel, this.kernel.length);
