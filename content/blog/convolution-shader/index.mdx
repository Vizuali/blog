---
title: "Image Processing: Convolution (Hardware)"
date: "2020-06-16T14:23:00.0000"
description: "Convolution implemented in p5js shaders"
---

_In this post we'll talk about convolution matrix implementation and gray scale._

import ImageShaderSketch from "../../../src/sketches/image-shader"

## Convolution matrix and Gray Scale

This is a convolution matrix implementation, don't know how to use it? Let's talk about it.

- __Gaussian Blur:__ Press 'b'
- __Box Blur:__ Press 'v'
- __Edge 1:__ Press 'e'
- __Edge 2:__ Press 'r'
- __Edge 3:__ Press 't'
- __Sharp:__ Press 's'
- __Gray Scale:__ Press 'g'
- __Luma 601:__ Press 'h'
- __Luma 709:__ Press 'j'
- __Identity:__ Press 'i' (This is useless but maybe someone want to see the same image twice)

_Note: Please be patient, without hardware this could be a little bit slow :D_

<div>{typeof window !== "undefined" && <ImageShaderSketch />}</div>

## References

- [Convolution 3x3 Shader Example](https://aferriss.github.io/p5jsShaderExamples/4_image-effects/4-15_convolution-kernel)
